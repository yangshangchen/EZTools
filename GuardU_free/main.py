# -*- coding: utf-8 -*-
"""守着你 (GuardU) - 安心版"""
import sys, os, json, smtplib, ssl, subprocess
...
DATA_DIR = os.path.join(os.path.expanduser("~"), ".guardu_free")
...
class FreeWindow(QMainWindow):
    ...
    def _send_alert_email(self):
        ...
        today = date.today().isoformat()
        sc = self.send_count
        if sc.get("date") == today and sc.get("count", 0) >= 1:
            QMessageBox.warning(self, "提示", "免费版每日仅可发送1次求助邮件\n如有紧急情况请直接拨打120")
            return
        body = "【守着你 · 安心版】紧急求助！\n\n用户已超过48小时未签到，可能存在危险。\n请尽快联系确认安全。\n\n-- 来自守着你安全助手"
